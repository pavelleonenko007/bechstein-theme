...
?>
<!DOCTYPE html>
<!-- This site was created in Webflow. https://www.webflow.com --><!-- Last Published: Wed Jul 13 2022 13:06:33 GMT+0000 (Coordinated Universal Time) --><html data-wf-page="62bc3fe7d9cc619b4c261599" data-wf-site="62bc3fe7d9cc6134bf261592">
    ...
    transform: rotateY(180deg);}
				...
				<div class="foo-mom">
					<div class="footer-container top-container">
						<div id="w-node-e1be876e-05a4-9245-628d-78602bcc79a4-2bcc79a3" class="footer-col _1 top-col"><a href="/whats-on" class="link-foo-big">What’s on?</a>
							...
							<div class="foo-marger"></div><a href="/festival" aria-current="page" class="link-foo-whats_last w-inline-block w--current"><img src="<?php echo get_template_directory_uri() ?>/images/62bc3fe7d9cc612f532615c7_Rectangle2039.jpg" loading="lazy" alt class="img-cover foters"><div class="text-block">Autumn Festival ‘22</div></a><a href="#" class="link-foo-small no-mob">Rachmaninov Days at Bechstein Hall</a></div>
		...
		<main class="wrapper">
			<section class="section wf-section">
				<div class="breadcrumbs-line"><a href="#" class="breadcrumbs-link">Home</a><a href="/whats-on" class="breadcrumbs-link">what’s on</a><a href="/festival" aria-current="page" class="breadcrumbs-link w--current"><?php the_title(); ?></a></div>
			</section>
			<section class="section wf-section">
				<div class="head-fest">
					<h1 class="h1-75-90">
					...
					<div class="p-35-45 date-text">
						<?php echo get_field('dates_of_festival') ?>
					</div>
				</div>
			</section>
			<section class="section wf-section">
				<div class="mom-fest-img"><img src="<?php $field = get_field('image'); if(isset($field['url'])){ echo($field['url']); }elseif(is_numeric($field)){ echo(wp_get_attachment_image_url($field, 'full')); }else{ echo($field); } ?>" loading="lazy" alt="<?php echo esc_attr($field['alt']); ?>" class="img-fest" title="<?php echo pathinfo($field['filename'])['filename'] !== $field['title'] ? esc_attr($field['title']) : ''; ?>"></div>
			</section>
			<section class="section wf-section">
				<div class="desc-fest">
					...
					<div class="festival-column no-mob">
						<div class="festival-styk">
							...
							<div class="p-17-25 italic">
								<?php echo get_field('note_about_festival') ?>
							</div>
						</div>
					</div>
					<div class="catalog-column fest-col">
						<div class="cms-ul-events">
							<?php if( have_rows('festival_dates') ){ ?>
							<div class="cms-tems">
								<?php global $parent_id; $parent_id = $loop_id; $loop_index = 0; $loop_title="Festival dates"; $loop_field = "festival_dates"; while( have_rows('festival_dates') ){ global $loop_id; $loop_index++; $loop_id++; the_row(); ?>
								<div class="cms-ul">
									<div class="cms-heading">
										<h2 class="h2-cms">
											<?php echo get_sub_field('festival_date') ?>
										</h2>
										<h2 class="h2-cms day">
											<?php echo get_sub_field('festival_day_of_week') ?>
										</h2>
									</div>
									<?php if( !isset($query_args)) $query_args = array('post_type' =>'event','posts_per_page'=>999, 'post__in' => get_sub_field('event_list')); if($args['paged']){ $query_args['paged'] = $args['paged']; }else{ $query_args['paged'] = get_query_var('page') ? get_query_var('page') : get_query_var('paged'); }$query = new WP_Query($query_args); if($query->have_posts()) : ?>
									<div class="cms-ul-events">
										...
										<div class="cms-li" data-content="query_item">
											<div class="cms-li_mom-img"><img src="<?php $field = get_field('small_image'); if(isset($field['url'])){ echo($field['url']); }elseif(is_numeric($field)){ echo(wp_get_attachment_image_url($field, 'full')); }else{ echo($field); } ?>" loading="lazy" alt="<?php echo esc_attr($field['alt']); ?>" class="cms-li_img" title="<?php echo pathinfo($field['filename'])['filename'] !== $field['title'] ? esc_attr($field['title']) : ''; ?>"></div>
											<div class="cms-li_content">
												...
												<div class="cms-li_actions-div"><a bgline="1" href="#" class="booktickets-btn"><strong>Book tickets</strong></a><a href="<?php the_permalink(); ?>" class="readmore-btn w-inline-block"><div>read more</div><div> →</div></a></div>
