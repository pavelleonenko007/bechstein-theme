...
?>
<!DOCTYPE html>
<!-- This site was created in Webflow. https://www.webflow.com --><!-- Last Published: Wed Jul 13 2022 13:06:33 GMT+0000 (Coordinated Universal Time) --><html data-wf-page="62bc3fe7d9cc61f0e526159f" data-wf-site="62bc3fe7d9cc6134bf261592">
    ...
    transform: rotateY(180deg);}
		...
		<main class="wrapper">
			...
			<section class="section wf-section">
				...
				<div class="mom-fest-img"><img src="<?php $field = get_field('main_image'); if(isset($field['url'])){ echo($field['url']); }elseif(is_numeric($field)){ echo(wp_get_attachment_image_url($field, 'full')); }else{ echo($field); } ?>" loading="lazy" alt="<?php echo esc_attr($field['alt']); ?>" class="img-fest" title="<?php echo pathinfo($field['filename'])['filename'] !== $field['title'] ? esc_attr($field['title']) : ''; ?>"></div>
				<?php } ?>
			</section>
			<section class="section wf-section">
				<div class="catalog-row m-revert">
					<div class="festival-column yvisit">
						<div class="yvisit-styk yvis _2">
							...
							<div>
								<?php global $parent_id; $parent_id = $loop_id; $loop_index = 0; $loop_title="Sidebar loop"; $loop_field = "sidebar_loop"; while( have_rows('sidebar_loop') ){ global $loop_id; $loop_index++; $loop_id++; the_row(); ?><a href="<?php echo get_sub_field('link') ?>" class="ui-yourvisit_link-in w-inline-block"><div class="p-20-30 med w20"><?php echo get_sub_field('header') ?></div><div class="p-17-25 mar10"><?php echo get_sub_field('description') ?></div></a>
								<?php } ?>
							</div>
							<?php } ?>
						</div>
					...
					<div class="yourvisit-column">
						<?php global $parent_id; $parent_id = $loop_id; $loop_index = 0; $loop_title="Site sections"; $loop_field = "site_sections"; while( have_rows('site_sections') ){ global $loop_id; $loop_index++; $loop_id++; the_row(); ?>
						...
						<div class="ui-boxoffice-block">
							...
							<div>
								<?php global $parent_id; $parent_id = $loop_id; $loop_index = 0; $loop_title="Blocks"; $loop_field = "blocks"; while( have_rows('blocks') ){ global $loop_id; $loop_index++; $loop_id++; the_row(); ?><a href="<?php echo get_sub_field('link') ?>" class="ui-yourvisit w-inline-block"><div class="ui-yourvisit_mom-div"><img src="<?php $field = get_sub_field('image'); if(isset($field['url'])){ echo($field['url']); }elseif(is_numeric($field)){ echo(wp_get_attachment_image_url($field, 'full')); }else{ echo($field); } ?>" loading="lazy" alt="<?php echo esc_attr($field['alt']); ?>" class="img-cover" title="<?php echo pathinfo($field['filename'])['filename'] !== $field['title'] ? esc_attr($field['title']) : ''; ?>"></div><div class="ui-yourvisit_content"><div class="p-35-45 w40"><?php echo get_sub_field('header') ?></div><div class="p-17-25 mar13"><?php echo get_sub_field('description') ?></div></div></a>
								<?php } ?>
							</div>
							<?php } ?>
						</div>
						...
						<div class="ui-boxoffice-block">
							<div class="wig-block">
								...
								<div class="cms-about">
									<?php global $parent_id; $parent_id = $loop_id; $loop_index = 0; $loop_title="Grid Blocks"; $loop_field = "grid_blocks"; while( have_rows('grid_blocks') ){ global $loop_id; $loop_index++; $loop_id++; the_row(); ?>
									<div class="ui-about">
										<div class="ui-about_mom-div"><img src="<?php $field = get_sub_field('image'); if(isset($field['url'])){ echo($field['url']); }elseif(is_numeric($field)){ echo(wp_get_attachment_image_url($field, 'full')); }else{ echo($field); } ?>" loading="lazy" alt="<?php echo esc_attr($field['alt']); ?>" class="img-cover" title="<?php echo pathinfo($field['filename'])['filename'] !== $field['title'] ? esc_attr($field['title']) : ''; ?>"></div>
										<div class="ui-yourvisit_content">
											...
											</div><a rel="nofollow" href="<?php echo get_sub_field('link_to_location') ?>" target="_blank" class="ui-about_link"><?php echo get_sub_field('link_to_location_text') ?></a></div>
									</div>
									<?php } ?>
								</div>
								<?php } ?>
						...
						<div class="ui-boxoffice-block">
							<div class="text-block-ui bloq2"><img src="<?php $field = get_sub_field('image'); if(isset($field['url'])){ echo($field['url']); }elseif(is_numeric($field)){ echo(wp_get_attachment_image_url($field, 'full')); }else{ echo($field); } ?>" loading="lazy" alt="<?php echo esc_attr($field['alt']); ?>" class="image-5" title="<?php echo pathinfo($field['filename'])['filename'] !== $field['title'] ? esc_attr($field['title']) : ''; ?>">
						...
						<div class="ui-boxoffice-block">
							...
							<div class="images-grid">
								<?php global $parent_id; $parent_id = $loop_id; $loop_index = 0; $loop_title="Gallery loop"; $loop_field = "gallery_loop"; while( have_rows('gallery_loop') ){ global $loop_id; $loop_index++; $loop_id++; the_row(); ?><a href="#" id="w-node-b48eefdc-4051-7d04-ab37-51fc79556990-e526159f" class="link-grid w-inline-block w-lightbox"><img src="<?php $field = get_sub_field('image'); if(isset($field['url'])){ echo($field['url']); }elseif(is_numeric($field)){ echo(wp_get_attachment_image_url($field, 'full')); }else{ echo($field); } ?>" loading="lazy" alt="<?php echo esc_attr($field['alt']); ?>" class="img-cover" title="<?php echo pathinfo($field['filename'])['filename'] !== $field['title'] ? esc_attr($field['title']) : ''; ?>"><script type="application/json" class="w-json"><?php
...
?></script></a>
						...
						<div class="ui-boxoffice-block">
							<?php if( !isset($query_args)) $query_args = array('post_type'=>'post', 'post_per_page'=>6); if($args['paged']){ $query_args['paged'] = $args['paged']; }else{ $query_args['paged'] = get_query_var('page') ? get_query_var('page') : get_query_var('paged'); }$query = new WP_Query($query_args); if($query->have_posts()) : ?>
							<div class="cms-press-ajax">
								<?php $rotation = 0; $group = 0; $post_index = 0; while($query->have_posts()) : $query->the_post(); $rotation === 0 ? $rotation = 1 : $rotation++; $group === 0 ? $group = 1 : $group++; $post_index++; ?> <a href="<?php the_permalink(); ?>" class="ui-pressrelease-a w-inline-block" data-content="query_item"><div class="p-20-30 w20"><?php echo get_the_date(get_option('date_format')); ?></div><div class="p-25-40 mar13"><?php the_title(); ?></div></a>
								<?php endwhile; ?>
							</div>
							<?php else : ?>
								...
								<div class="payment-tabs">
									<div data-current="Tab 1" data-easing="ease" data-duration-in="300" data-duration-out="100" class="tabs w-tabs">
										...
										<div class="tabs-menu w-tab-menu">
											<?php global $parent_id; $parent_id = $loop_id; $loop_index = 0; $loop_title="Tabs"; $loop_field = "tabs"; while( have_rows('tabs') ){ global $loop_id; $loop_index++; $loop_id++; the_row(); ?><a data-w-tab="Tab <?php echo get_row_index() ?>" class="tab-payments w-inline-block w-tab-link <?php if(get_row_index() === 1) echo 'w--current'; ?>"><div><?php echo get_sub_field('tab_header') ?></div></a>
											<?php } ?>
										</div>
										<?php } ?>
										<?php if( have_rows('tabs') ){ ?>
										<div class="tabs-content w-tab-content">
											<?php global $parent_id; $parent_id = $loop_id; $loop_index = 0; $loop_title="Tabs"; $loop_field = "tabs"; while( have_rows('tabs') ){ global $loop_id; $loop_index++; $loop_id++; the_row(); ?>
